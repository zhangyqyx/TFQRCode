...
#import "TFCodeAlbumManager.h"
#import <AVFoundation/AVFoundation.h>
#import <Photos/Photos.h>

@interface TFCodeAlbumManager () <UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation TFCodeAlbumManager
static TFCodeAlbumManager *_instance;
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController *)picker {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TF_codeAlbumManagerDidCancelWithImagePickerController:)]) {
        [self.delegate TF_codeAlbumManagerDidCancelWithImagePickerController:self];
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if (features.count == 0) {
        ...
        [self.currentVC dismissViewControllerAnimated:YES completion:^{
            if (self.delegate && [self.delegate respondsToSelector:@selector(TF_codeAlbumManager:didFinishPickingMediaWithResult:)]) {
                [self.delegate TF_codeAlbumManager:self didFinishPickingMediaWithResult:self.detectorString];
            }
        }];
    }
}
