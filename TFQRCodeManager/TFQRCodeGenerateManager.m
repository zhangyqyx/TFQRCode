//
//  TFQRCodeGenerateManager.m
//  TFQRCode
//
//  Created by 张永强 on 2018/1/23.
//  Copyright © 2018年 张永强. All rights reserved.
//

#import "TFQRCodeGenerateManager.h"

@implementation TFQRCodeGenerateManager
#pragma mark - 生成简单二维码图片
+ (UIImage *)TF_generateQRCodeWithData:(NSData *)data
...
+ (UIImage *)TF_generateQRCodeWithStr:(NSString *)str
                            sizeWidth:(CGFloat)sizeWidth {
    NSData *infoData = [str dataUsingEncoding:NSUTF8StringEncoding];
    return [self TF_generateQRCodeWithData:infoData sizeWidth:sizeWidth];
}

#pragma mark - 带logo的二维码图片o

+ (UIImage *)TF_generateQRCodeWithLogoStr:(NSString *)str
                            logoImageName:(NSString *)logoImageName
                     logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    return [self TF_generateQRCodeWithLogoData:[str dataUsingEncoding:NSUTF8StringEncoding]
                                 logoImageName:logoImageName
                          logoScaleToSuperView:logoScaleToSuperView];
}

+ (UIImage *)TF_generateQRCodeWithLogoData:(NSData *)data
                             logoImageName:(NSString *)logoImageName
                      logoScaleToSuperView:(CGFloat)logoScaleToSuperView {
    // 获得滤镜输出的图像
    CIImage *outputImage = [self getCIImageWithData:data];
...
+ (UIImage *)TF_generateWithColorQRCodeStr:(NSString *)str
                           backgroundColor:(CIColor *)backgroundColor
                                 mainColor:(CIColor *)mainColor {
    return   [self  TF_generateWithColorQRCodeData:[str dataUsingEncoding:NSUTF8StringEncoding] backgroundColor:backgroundColor mainColor:mainColor];
}
+ (UIImage *)TF_generateWithColorQRCodeData:(NSData *)data
                            backgroundColor:(CIColor *)backgroundColor
                                  mainColor:(CIColor *)mainColor {
    // 获得滤镜输出的图像
    CIImage *outputImage = [self getCIImageWithData:data];
